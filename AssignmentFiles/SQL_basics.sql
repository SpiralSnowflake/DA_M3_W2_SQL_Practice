...
FROM orders 
ORDER BY order_datetime -- by default ORDER BY sorts in asc-ascending order
limit 5;
-- ====================================================================================================================
...
SELECT store_id, name as 'Store Name', CONCAT(city, '/', state) AS City_State
FROM stores;
-- ====================================================================================================================
-- Q7) From orders, show order_id, status, and a computed column total_items
--     that counts how many items are in each order.
SELECT  order_items.order_id, orders.status, COUNT(order_items.order_id) AS total_items
FROM orders,order_items
WHERE orders.order_id = order_items.order_id
GROUP BY order_id;


-- total items by order without status, for comparing the results above 
SELECT order_items.order_id, COUNT(order_items.order_id) AS total_items
FROM order_items
GROUP BY order_id;
